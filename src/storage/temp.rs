...
impl TempStorage {
    ...
    /// target_file: 目标文件句柄
    pub fn merge(&self, ids: &[String], mut target_file: File) -> anyhow::Result<()> {
        // 先验证都是有效的 id
        for id in ids {
        ...
        for id in ids {
            ...
            let chunk_content = fs::read(&chunk_path)?;
            target_file.write_all(&chunk_content)?;
        }
        ...
        for id in ids {
            let chunk_path = self.path.join(id);
            let _ = fs::remove_file(&chunk_path); // 忽略删除错误
        }
