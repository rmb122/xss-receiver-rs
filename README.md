...
HTTP 和 DNS 日志页面支持表达式过滤. 点击过滤按钮展开输入框, 输入时提供语法高亮, 并按光标位置提示字段, 运算符和可选值. Ctrl+Space 手动唤出提示, Enter 或 Tab 接受补全, Esc 关闭提示. 提示关闭时按 Enter 或点击应用提交, 清空可恢复全部日志. 输入框右侧的问号按钮提供语法与字段帮助.
