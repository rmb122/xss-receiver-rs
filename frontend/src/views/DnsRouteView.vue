      ...
      <v-card>
        ...
        <v-card-text>
          ...
          <v-text-field
            ...
            label="匹配域名"
            :placeholder="form.pattern_kind === 'PLAIN' ? 'example.com' : '.*\\.example\\.com'"
            variant="outlined"
            density="compact"
            class="mb-2"
          />
...
async function fetchRoutes() {
  loading.value = true
  try {
    routes.value = await getAllDnsRoutes()
    nextTick(() => {
...
function openEditDialog(route: DnsRoute) {
...
async function handleDelete(route: DnsRoute) {
  ...
  await deleteDnsRoute({ route_id: route.id })
